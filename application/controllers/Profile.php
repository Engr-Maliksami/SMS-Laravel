...
class Profile extends My_Controller {
    ...
    public function update() {
        ...
        if ($_POST) {
              /** vipul start **/	
			     $old_user =  $this->db->get_where('users', array('id' => $this->input->post('id')))->row();	
            ...
            if ($this->input->post('user_type') == 'system_admin') {
                $updated = $this->profile->update('system_admin', $data, array('id' => $this->input->post('id')));
                 /** vipul start **/	
				     $existing_details =  $this->db->get_where('system_admin', array('id' => $this->input->post('id')))->row();	
				    /** vipul end **/
            }
            ...
            if ($this->input->post('user_type') == 'employee') {
                ...
                /** vipul start **/	
				     $existing_details = $this->db->get_where('employees', array('id' => $this->input->post('id')))->row();	
				    /** vipul end **/
            }
            ...
            if ($this->input->post('user_type') == 'teacher') {
                ...
                /** vipul start **/	
				     $existing_details = $this->db->get_where('teachers', array('id' => $this->input->post('id')))->row();	
				    /** vipul end **/
            }
            if ($this->input->post('user_type') == 'guardian') {
                $updated = $this->profile->update('guardians', $data, array('id' => $this->input->post('id')));
                     /** vipul start **/	
				     $existing_details = $this->db->get_where('guardians', array('id' => $this->input->post('id')))->row();	
				    /** vipul end **/
            }
            if ($this->input->post('user_type') == 'student') {
                $updated = $this->profile->update('students', $data, array('id' => $this->input->post('id')));
                  /** vipul start **/	
				    $existing_details = $this->db->get_where('students', array('id' => $this->input->post('id')))->row();	
				    /** vipul end **/
            }
            /** vipul start **/	
			    if($existing_details) {	
				    $learn_db = $this->load->database('learn', TRUE);	
				    $existing_user =  $this->db->get_where('users', array('id' => $this->input->post('id')))->row();	
				    $learn_db->where('username', $old_user->username);	
				    $learn_db->set('name', $existing_details->name);	
				    $learn_db->set('username', $existing_user->username);	
				    $learn_db->set('email', $existing_details->email);	
				    $learn_db->set('slug', $existing_user->username);	
				    $learn_db->set('phone', $existing_details->phone);	
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $this->profile->update('users', $data, array('id' => logged_in_user_id()));
                 /** vipul start **/	
                 $learn_db = $this->load->database('learn', TRUE);	
                 $existing_user =  $this->db->get_where('users', array('id' => logged_in_user_id()))->row();	
                 $existing_username = $existing_user->username;	
     
                 $learn_pass =  password_hash($this->input->post('password'), PASSWORD_BCRYPT, ['cost' => 10,]);	
                 $learn_db->query("update users set password = '$learn_pass', updated_at = now() where username = '$existing_username'");	
