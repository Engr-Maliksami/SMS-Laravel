...
class Payment extends My_Controller {
    ...
    * ********************************************************** */
    private function _prepare_payment_validation() {
        ...
        elseif($this->input->post('payment_method') == 'pesapal'){
            ...
            $this->form_validation->set_rules('pesapal_email', $this->lang->line('email'), 'trim|required');
        }
    ...
    * ********************************************************** */
    public function amount() {
        ...
        $due_amount      = $invoice->net_amount - $invoice->paid_amount;
        $due_amount = $due_amount * 100; // change
    ...
    * ********************************************************** */
    public function payment_method() {
        ...
        } elseif ($this->input->post('payment_method') == 'pesapal' && $payment_method->pesapal_status == 1) {
          if ($payment_method->pesapal_consumer_key && $payment_method->pesapal_consumer_secret) {
              return TRUE;
          }else{
            $this->form_validation->set_message("payment_method", $this->lang->line('input_valid_payment_setting'));
              return FALSE;
    ...
    * ********************************************************** */
    private function _get_posted_payment_data() {
        ...
        if($this->input->post('payment_method') == 'pesapal'){
           $data['pesapal_email'] = $this->input->post('pesapal_email');
       }
    ...
    * ********************************************************** */
    ...
    public function pesapal($data)
    {
        ...
        if($payment_setting->pesapal_extracharge > 0){
            $this->data['amount'] = ($data['amount'] + ($payment_setting->pesapal_extracharge/100*$data['amount']))*100;
        }

        $this->data['pesapal_consumer_key'] = $payment_setting->pesapal_consumer_key;
        $this->data['pesapal_consumer_secret'] = $payment_setting->pesapal_consumer_secret;
        $this->data['pesapal_demo'] = $payment_setting->pesapal_demo;
        $this->data['email'] = $data['pesapal_email']; // customer email
        $this->data['reference'] = uniqid().'-'.$data['invoice_id']; // unique reference
        $this->data['invoice_id'] = $data['invoice_id'];
        // print_r($this->data); die();
        $this->load->view('payment/pesapal', $this->data);
    ...
    public function pesapal_response() {
      ...
      if (!empty($invoice)) {
          ...
          $data['transaction_id'] = $pesapal_tracking_id;
          // $data['stack_reference'] = $reference;
          $data['note'] = 'Pesapal Payment';
          $data['status'] = 1;
          $data['academic_year_id'] = $school->academic_year_id;
          $data['payment_date'] = date('Y-m-d');
          $data['created_at'] = date('Y-m-d H:i:s');
