                    ...
                    <div class="tab-content">
                        ...
                        <div class="x_content">
                           <?php echo form_open(site_url('accounting/payment/paid/'.$invoice_id), array('name' => 'add', 'id' => 'add', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                    ...
                                    <select  class="form-control col-md-7 col-xs-12"  name="payment_method"  id="payment_method" required="required" onchange="check_payment_method(this.value);">
                                        ...
                                        <?php foreach($payments as $key=>$value ){ ?>
                                            <?php if($this->session->userdata('role_id') == GUARDIAN || $this->session->userdata('role_id') == STUDENT){ ?>
                                                <?php if(in_array($key, array('paypal', 'payumoney', 'ccavenue', 'paytm', 'paystack', 'pesapal'))){ ?>
                                                    <option value="<?php echo $key; ?>" <?php if(isset($post) && $post['payment_method'] == $key){ echo 'selected="selected"';} ?>><?php echo $value; ?></option>
                                                <?php } ?>
                                            <?php }else{ ?>
                                                    <?php if(!in_array($key, array('paypal', 'payumoney', 'ccavenue', 'paytm', 'stripe', 'paystack', 'pesapal'))){ ?>
                                                     <option value="<?php echo $key; ?>" <?php if(isset($post) && $post['payment_method'] == $key){ echo 'selected="selected"';} ?>><?php echo $value; ?></option>
                                                <?php } ?>
                                            <?php } ?>
                                        <?php } ?>
                            ...
                            <!-- For Pesapal Start-->
                       <div class="display fn_pesapal"  style="<?php if(isset($post) && $post['payment_method'] == 'pesapal'){ echo 'display:block;';} ?>">

                           <div class="item form-group">
                               <label class="control-label col-md-3 col-sm-3 col-xs-12" for="pesapal_email"><?php echo $this->lang->line('email'); ?> <span class="required">*</span>
                               </label>
                               <div class="col-md-6 col-sm-6 col-xs-12">
                                   <input  class="form-control col-md-7 col-xs-12"  name="pesapal_email"  id="pesapal_email" value="" placeholder="<?php echo $this->lang->line('email'); ?>"  type="text" autocomplete="off">
                                   <div class="help-block"><?php echo form_error('pesapal_email'); ?></div>
                               </div>
                           </div>
 ...
 <script type="text/javascript">
    ...
    function check_payment_method(payment_method) {
            ...
            if (payment_method == "stripe") {
                ...
                $('#pesapal_email').prop('required', false);
              ...
              $('#expire_year').prop('required', false);
              $('#stack_email').prop('required', false);
              $('#pesapal_email').prop('required', true);
