    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $dates = explode("-",$data['session_year']);
                
                $first =explode(" ",$dates[0]);
                $first_month_name = $first[0];
                $first_year = $first[1];
                $last =explode(" ",trim($dates[1]));
                $last_month_name = $last[0];
                $last_year = $last[1];
                $first_month_number = date('m', strtotime($first_month_name));
                $first_date = $first[1].'-'.$first_month_number.'-01';
                $last_month_number = date('m', strtotime($last_month_name));
                $last_date = $last[1].'-'.$last_month_number.'-01';
               
                //echo $first_date.'='.$last_date; die();
