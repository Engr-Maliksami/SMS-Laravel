...
class School extends MY_Controller {
    ...
    * ********************************************************** */ 
    ...
    private function _get_posted_school_data() {
        ...
        if ($_FILES['frontend_page']['name']) {
            $data['frontend_page'] = $this->_upload_frontend_page();
        }
        return $data;
    }
    ...
    * ********************************************************** */
    ...
    /*****************Function _upload_frontend_page**********************************
    * @type            : Function
    * @function name   : _upload_frontend_page
    * @description     : Process to upload school front end logo in the server                  
    *                     and return logo name   
    * @param           : null
    * @return          : $logo string value 
    * ********************************************************** */
    private function _upload_frontend_page() {

        $prevoius_logo = @$_POST['frontend_page_prev'];
        $logo_name = $_FILES['frontend_page']['name'];
        $logo_type = $_FILES['frontend_page']['type'];
        $logo = '';
        ...
        if ($logo_name != "") {
            if ($logo_type == 'image/jpeg' || $logo_type == 'image/pjpeg' ||
                ...
                $destination = 'assets/uploads/frontend_page/';

                $file_type = explode(".", $logo_name);
                $extension = strtolower($file_type[count($file_type) - 1]);
                $logo_path = time().'-school-front-page.' . $extension;

                copy($_FILES['frontend_page']['tmp_name'], $destination . $logo_path);
   ...
   * ********************************************************** */
    public function delete($id = null) {        
        ...
        if ($this->school->delete('schools', array('id' => $id))) {
            ...
            if (file_exists( $destination.$school->frontend_page)) {
                @unlink($destination.$school->frontend_page);
            }
