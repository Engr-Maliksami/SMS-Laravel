<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div class="x_title">
                <h3 class="head-title"><i class="fa fa-file-text-o"></i><small> All <?php echo $this->lang->line('student'); ?> <?php echo $this->lang->line('result_card'); ?></small></h3>
                ...
                <?php foreach ($students as $std) { ?>
                        ...
                        <table id="datatable-responsive" class="table table-striped_ table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <?php
                                    $count_all = count($exam_types);
                                    $total_exams= array();
                                    foreach ($exam_types as $types) {
                                        $total_exams[] = $types->exam_type_name;

                                    ?>
                                        <th colspan="2"><?php echo $types->exam_type_name; ?></th>
                                    <?php } ?>
                                ...
                                <tr>
                                    <?php foreach ($exam_types as $types) { ?>
                                    ...
                                    <?php foreach ($exams as $ex) { ?>
                                        ...
                                        ?>
                                            ...
                                            <?php foreach ($exam_subjects as $obj) { ?>
                                                ...
                                                $obtained_marks = get_exam_details($school_id, $ex->id, $class_id, $academic_year_id, $std->id);
                                                ?>
                                                ...
                                                <?php $lh       = get_lowet_height_mark($school_id, $academic_year_id, $ex->id, $class_id, $std->section_id, $obj->subject_id); ?>
                                                <?php $position = get_position_in_subject($school_id, $academic_year_id, $ex->id, $class_id, $std->section_id, $obj->subject_id, $obj->obtain_total_mark);
                                                $obtained_marks = get_exam_details_by_subject($school_id, $ex->id, $class_id, $academic_year_id, $obj->subject_id, $std->id);
                                                $count_selected = count($obtained_marks);
                                                ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <?php
                                                    //echo '<pre>';print_r($total_exams);
                                                    //echo '<pre>';print_r($obtained_marks);
                                                    //echo '<pre>';print_r($exam_types);
                                                    $total_count = $count_all - $count_selected;
                                                    $i = 0;
                                                    
                                                    ?>
                                                    <?php foreach ($exam_types as $key => $types) {
                                                    
                                                        while ($i < count($total_exams)) {
                                                            if ($total_exams[$i] == $obtained_marks[$key]->exam_type_name) {
                                                                if ($ex->id == $obtained_marks[$key]->exam_id) {
                                                                    $exam_name =  $ex->id . '_' . $total_exams[$i];
                                                                    $obtained[$exam_name] += $obtained_marks[$key]->marks_obtained;
                                                                    $maximum[$exam_name] += $obtained_marks[$key]->max_marks;
                                                                }
                                                                $i++;
                                                        ?>
                                                                <td><?php echo $obtained_marks[$key]->max_marks; ?></td>
                                                                <td><?php echo $obtained_marks[$key]->marks_obtained; ?></td>
                        ...
                        <table id="datatable-responsive" class="table table-striped_ table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                            ...
                            ?>
                                ...
                                <?php foreach ($exams as $ex) { ?>
                                    ...
                                    $exam_subjects = get_subject_list($school_id, $academic_year_id, $ex->id, $class_id, $std->$section_id, $std->id); 
                                     $mark = get_exam_wise_markt($school_id, $academic_year_id, $ex->id, $class_id, $std->section_id, $std->id); ?>
                                    <?php
                                    foreach ($exam_subjects as $sub){
                                        $obtained_marks = get_exam_details_by_subject($school_id, $ex->id, $class_id, $academic_year_id,$sub->subject_id,$std->id);
                                        foreach($obtained_marks as $result){
                                            $exam_type_details_result[$result->exam_id.'_'.$result->subject_id.'_'.$result->exam_type_name.'_maxmarks'] =  $result->max_marks;
    
                                            $exam_type_details_result[$result->exam_id.'_'.$result->subject_id.'_'.$result->exam_type_name.'_obtain'] =  $result->marks_obtained;
                                        }
                                    }

                                    $obtained_marks = get_exam_details($school_id, $ex->id, $std->class_id, $academic_year_id,$std->id);
                                   

                                     $obtain_total_mark = $exam->total_obtain_mark; 

                                     $rank = get_position_in_exam($school_id, $academic_year_id, $ex->id, $class_id, $std->section_id, $obtain_total_mark); ?>
                                    <?php $exam_lh = get_lowet_height_result($school_id, $academic_year_id, $ex->id, $class_id, $std->section_id, $std->id); ?>

                                    <tr>
                                        <td><?php echo $count++;  ?></td>
                                        <td><?php echo ucfirst($ex->title); ?></td>
                                        <?php for ($i = 0; $i < count($total_exams); $i++) {
                                            $exa = $ex->id . '_' . $total_exams[$i];
                                        ?>
                                            <td><?php if (isset($maximum[$exa])) {
                                                    echo $maximum[$exa];
                                                } else {
                                                    echo 'N/A';
                                                } ?></td>
                                        ...
                                        <?php   }
                                        ?>
                                         <td><?php echo $exam->total_mark;  ?></td>
                    ...
                    <table class="table table-striped_ table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <?php $class_position = get_student_position($school_id, $academic_year_id, $class_id, $std->id); ?>
                            <?php $section_position = get_student_position($school_id, $academic_year_id, $class_id, $std->id, $std->section_id); ?>
                            <?php $final_result = get_final_result($school_id, $academic_year_id, $class_id, $std->section_id, $std->id); ?>
                            <tr>
