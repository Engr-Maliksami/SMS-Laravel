...
class Student_Model extends MY_Model {
    ...
    public function update_admission_no($id){ 
        
        $sql = "Update students set admission_no = $id  WHERE `id` =$id";
   ...
   // echo $this->db->last_query();
    ...
    function max_participant_check($school_id){ 
        $ci = & get_instance();    
        $ci->db->select('COUNT(S.id) as total_students');
        $ci->db->from('students AS S');     
        //$ci->db->join('max_participant_allowed AS M', 'M.school_id = S.school_id'); 
       ...
       function get_user_id($school_id){ 
        $ci = & get_instance();    
        $ci->db->select('S.user_id,S.base_url_server');
        $ci->db->from('global_setting AS S');     
        //$ci->db->where('S.school_id', $school_id);
        //$ci->db->get();
        return $ci->db->get()->row(); 
        //echo $ci->db->last_query();die(); 
