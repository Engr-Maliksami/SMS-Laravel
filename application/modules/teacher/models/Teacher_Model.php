...
class Teacher_Model extends MY_Model {
    ...
    function max_participant_check($school_id){ 
        $ci = & get_instance();    
        $ci->db->select('COUNT(T.id) as total_teachers,M.teachers_allowed');
        $ci->db->from('teachers AS T');     
        $ci->db->join('max_participant_allowed AS M', 'M.school_id = T.school_id'); 
        $ci->db->where('T.school_id', $school_id);
        //$ci->db->get();
