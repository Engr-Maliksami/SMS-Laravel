...
class DashboardController extends Controller
{
    ...
    public function index()
    {
      ...
      $data['right_bar_data']     = array('chart_data' => $data['chart_data'] );
		...
		$user = Auth::user();
		                $record = User::where('slug', $user->slug)->first();
                $options = [];
                $quiz_categories = QuizCategory::all();
                $lms_categories = LmsCategory::all();
                $options['quiz_categories'] = [];
                $options['lms_categories']  = [];
                foreach($quiz_categories as $q)
                        $options['quiz_categories'][] = $q->id;

                foreach($lms_categories as $l)
                        $options['lms_categories'][] = $l->id;
